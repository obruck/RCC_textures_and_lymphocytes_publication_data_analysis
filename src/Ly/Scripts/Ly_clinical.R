...
tcga_kirc$normal = ifelse(tcga_kirc$`texture_normal_%` < 1, 0, 1)
...
tcga_kirc <- tcga_kirc %>% dplyr::mutate(therapy_outcome = factor(therapy_outcome, levels = c("Complete Remission/Response", "Stable Disease", "Progressive Disease", NA)))
...
to_hotencode_var <- to_hotencode_var %>%
  dplyr::select(-c(Age_Low, gender_FEMALE, vital_status_Alive, LDH_Normal, ECOG_0, `Karnofsky_≤80`)) %>%
  dplyr::select(-one_of(colnames(tcga_kirc)))
### Join
to_hotencode_var <- cbind(tcga_kirc %>% dplyr::select(tcga_id), to_hotencode_var)
tcga_kirc <- tcga_kirc %>% dplyr::left_join(to_hotencode_var)
  ...
  for (two1 in b) {
    ...
    if (length(unique(tcga_kirc1$two1)) == 2) {
      ...
      g <- ggplot(tcga_kirc1, aes(x = two1, y = FC)) +
        ...
        labs(y=paste0("Lymphocyte proportion in ", texture1, "(%)"), x=toupper(gsub("_mutation", "", two1))) +
        theme_bw() +
...
tcga_kirc1 <- tcga_kirc0 %>%
  dplyr::select(tcga_id, vital_status_Dead, `texture_normal_%`, !!textures) %>%
